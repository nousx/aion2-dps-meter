class DpsApp {
  constructor() {
    ...
    // Smart polling - adapt speed based on combat state
    this.POLL_MS_COMBAT = 50;   // 20 FPS during combat
    this.POLL_MS_IDLE = 200;     // 5 FPS when idle
    this.POLL_MS = this.POLL_MS_IDLE;
    this.WINDOW_TITLE_POLL_MS = 30000;
    this.USER_NAME = "";
    this.onlyShowUser = false;
    this.debugLoggingEnabled = false;
    this.packetLoggingEnabled = false;
    this.storageKeys = {
      ...
      packetLogging: "dpsMeter.packetLoggingEnabled",
    };

    this.dpsFormatter = new Intl.NumberFormat("en-US", {
      maximumFractionDigits: 0,  // ไม่มีทศนิยม
  ...
  start() {
    ...
    this.meterUI = createMeterUI({
      ...
      formatNumber: (n) => this.formatAbbreviatedNumber(n),
    });
    ...
    // Removed: No longer needed - displaying both damage and DPS simultaneously
  ...
  startPolling() {
    ...
    const poll = () => {
      ...
      // Auto-refresh details panel if open
      if (this.detailsUI?.isOpen?.()) {
        this.detailsUI.refresh();
      }

      // Adaptive polling rate
      const isInCombat = this._battleTimeVisible && this.lastSnapshot?.length > 0;
      const newPollMs = isInCombat ? this.POLL_MS_COMBAT : this.POLL_MS_IDLE;

      if (newPollMs !== this.POLL_MS) {
        this.POLL_MS = newPollMs;
  ...
  fetchDps() {
    ...
    // Update details panel with latest row data if it's open
    if (this.detailsUI?.isOpen?.() && this.detailsUI.lastRow) {
      const openedRowId = this.detailsUI.lastRow.id;
      const updatedRow = rowsToRender.find(r => r.id === openedRowId);
      if (updatedRow) {
        this.detailsUI.lastRow = updatedRow; // Update lastRow reference
      }
    }
    ...
    if (window.performanceMonitor) {
      ...
      window.performanceMonitor.recordPoll(renderTime);
      window.performanceMonitor.recordInterpolation(window.numberInterpolator?.isRunning || false);
    }
  }
  ...
  buildRowsFromMapObject(mapObj) {
    ...
    for (const [id, value] of Object.entries(mapObj || {})) {
      ...
      const name = hasNickname ? nickname : `Player ${id}`;

      const dpsRaw = isObj ? value.dps : value;
      const dps = Math.trunc(Number(dpsRaw));
      const burstDps = Math.trunc(Number(isObj ? value.burstDps : 0));
      ...
      // Crit rate (소수점 한자리)
      const critRaw = isObj ? Number(value.totalCritPct) : NaN;
      const totalCritPct = Number.isFinite(critRaw)
        ? Math.round(critRaw * 10) / 10
        : 0;

      // Back attack rate (소수점 한자리)
      const backRaw = isObj ? Number(value.totalBackPct) : NaN;
      const totalBackPct = Number.isFinite(backRaw)
        ? Math.round(backRaw * 10) / 10
        : 0;
  ...
  async getDetails(row) {
    // Use mock data if available (for UI testing)
    if (window.mockGetDetails) {
      return await window.mockGetDetails(row);
    }
    ...
    for (const [code, value] of Object.entries(detailObj)) {
      ...
      const pushSkill = ({
        ...
        skills.push({
          ...
          dmg: dmgInt,
          specialtySlots: Array.isArray(specialtySlots) ? specialtySlots : [],
        });
      };
      ...
      pushSkill({
        ...
        specialtySlots: value.specialtySlots,
      });
  ...
  setupSettingsPanel() {
    ...
    this.packetLoggingCheckbox = document.querySelector(".packetLoggingCheckbox");
    ...
    const storedPacketLogging = this.safeGetSetting(this.storageKeys.packetLogging) === "true";
    ...
    this.setPacketLogging(storedPacketLogging, { persist: false, syncBackend: true });
    ...
    if (this.packetLoggingCheckbox) {
      this.packetLoggingCheckbox.checked = this.packetLoggingEnabled;
      this.packetLoggingCheckbox.addEventListener("change", (event) => {
        const isChecked = !!event.target?.checked;
        this.setPacketLogging(isChecked, { persist: true, syncBackend: true });
      });
    }
  ...
  setPacketLogging(enabled, { persist = false, syncBackend = false } = {}) {
    this.packetLoggingEnabled = !!enabled;
    if (this.packetLoggingCheckbox && document.activeElement !== this.packetLoggingCheckbox) {
      this.packetLoggingCheckbox.checked = this.packetLoggingEnabled;
    }
    if (persist) {
      this.safeSetSetting(this.storageKeys.packetLogging, String(this.packetLoggingEnabled));
    }
    if (syncBackend) {
      window.javaBridge?.setPacketLoggingEnabled?.(this.packetLoggingEnabled);
    }
  }
  ...
  formatAbbreviatedNumber(value) {
    ...
    for (const unit of units) {
      if (abs >= unit.value) {
        const scaled = (n / unit.value).toFixed(1);
  ...
  // Removed: No longer needed - meter.js now handles damage and DPS directly
  // getMetricForRow(row) { ... }
...
window.dpsApp = dpsApp; // Expose for mock data testing
