...
class DpsCalculator(private val dataStorage: DataStorage) {
    ...
    companion object {
        // Load skill code data from JSON configuration
        private val skillCodeData = SkillCodeLoader.loadSkillCodeData()

        // Legacy field names for backward compatibility
        val POSSIBLE_OFFSETS: IntArray = skillCodeData.possibleOffsets.toIntArray()
        val SKILL_MAP: Map<Int, String> = skillCodeData.skillNames
        val SKILL_CODES: IntArray = skillCodeData.skillCodes.toIntArray()

        // Removed hardcoded data - now loaded from src/main/resources/data/skill_codes.json
        /*
        val SKILL_MAP_OLD = mapOf(
            /*
        Cleric
         */
        ...
        */
        ...
        // SKILL_CODES also removed - loaded from JSON
        /*
        val SKILL_CODES_OLD: IntArray =
            intArrayOf(
                100051,
                100055,
                17010000,
    ...
    fun getDps(): DpsData {
        ...
        pdps.forEach { pdp ->
            ...
            pdp.setSkillCode(inferOriginalSkillCode(pdp.getSkillCode1(), pdp.getTargetId(), pdp.getActorId(), pdp.getDamage(), pdp.getHexPayload()) ?: pdp.getSkillCode1())
            dpsData.map[uid]!!.processPdp(pdp)
            if (dpsData.map[uid]!!.job == "") {
                val origSkillCode = inferOriginalSkillCode(pdp.getSkillCode1(), pdp.getTargetId(), pdp.getActorId(), pdp.getDamage(), pdp.getHexPayload()) ?: -1
        ...
        while (iterator.hasNext()) {
            ...
            if (data.job == "") {
                ...
                // Use rolling window DPS for accurate real-time calculation
                data.dps = data.calculateRollingDPS(currentTime)
                data.burstDps = data.calculateBurstDPS(currentTime)
                data.damageContribution = data.amount / totalDamage * 100
                data.totalCritPct = data.calculateCritRate()
                data.totalBackPct = data.calculateBackRate()
            }
        }
    ...
    private fun inferOriginalSkillCode(
        ...
        logger.debug("Failed to infer skill code payload={}", payloadHex)
